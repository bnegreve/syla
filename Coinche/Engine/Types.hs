...
data Rank = C_7 | C_8 | C_9 | C_J | C_Q | C_K | C_10 | C_As
            deriving (Show,Enum,Eq,Generic,Ord)
...
type Distribution = [(Card,Double)]
data GameStats = GameStats {_gsDistribution :: M.Map Player [Distribution]}
  deriving (Generic,Show)
 

instance Monoid GameStats where
  mempty = GameStats $ M.empty
  mappend (GameStats m1) (GameStats m2) = GameStats $ M.unionWith (++) m1 m2
